...
{
    Cell** _board = gameBoard.getBoard();
	for (size_t i = 0; i < 10; i++) {
		switch (ships[i]->getTypeShip()) {
		case TypeShip::SingleDeckShip:{
			SingleDeck* sd = (SingleDeck*)ships[i];
			Deck d = sd->getDeck();
			_board[d.getX()][d.getY()].setStatusCell(StatusCell::Occupied);
			}break;
		case TypeShip::DoubleDeckShip: {
			DoubleDeck* sd = (DoubleDeck*)ships[i];
		...
		case TypeShip::ThreeDeckShip: {
			ThreeDeck* sd = (ThreeDeck*)ships[i];
			Deck d_1 = sd->getDeck_1();
			Deck d_2 = sd->getDeck_2();
     ...
     gameBoard.setCellsBoard(_board);
}
